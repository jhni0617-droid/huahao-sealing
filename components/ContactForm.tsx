  ...
  return (
    <form onSubmit={handleSubmit} className="card p-6 md:p-8 space-y-5">
      <p className="text-sm text-muted">
        填写基本信息，我们将在24小时内与您联系。
      </p>
      <div className="grid sm:grid-cols-2 gap-5">
        <div>
          <label className="form-label">姓名 *</label>
          <input required className="form-input" placeholder="您的姓名" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        </div>
        <div>
      ...
      <div>
        ...
        <textarea required rows={4} className="form-input resize-y" placeholder="请描述您的应用工况、需求或数量..." value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} />
      </div>
